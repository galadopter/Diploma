...
\input{setup/fonts_linux}% не забудьте закомментировать \input{fonts_windows}
...
\AtBeginEnvironment{figure}{\setlength{\intextsep}{0.1\baselineskip}}

\usepackage{fancyhdr}
% \usepackage[margin=1.0in,headheight=200pt]{geometry}
...
\newcommand{\appendice}[1]{%
  ...
  \textbf{\MakeUppercase{Приложение \thesection} \\ (обязательное)\\}%
  % \newline%
  \textbf{\\#1}%
  \end{center}%
  \thispagestyle{plain}%
  ...
  \fancyhead[CO,CE]{\vspace{1.5em}Продолжение приложения \thesection\vspace{15pt}}%
  \addcontentsline{toc}{section}{Приложение \thesection~(обязательное) #1}%
}
\renewcommand{\headrulewidth}{0pt}
% \setlength{\headheight}{500pt} 
...
\captionsetup[figure]{labelformat=stbfigure,justification=centering,skip=15pt,belowskip=10pt}
